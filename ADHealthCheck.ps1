...
.NOTES
    ...
    Changelog:  - CHANGE: Die automatisch angelegten Reverse-Systemzonen
                ...
                - FIX: Der Self-Updater laedt jetzt auch templates/report.style.css
                  ...
                  Stilaenderung seit Bestehen des Updaters ist bei
                  ausgelieferten Installationen NIE ANGEKOMMEN. Im
                  Entwicklungsverzeichnis lag immer die aktuelle Datei,
                  deshalb fiel es nicht auf. Ein neuer Testblock prueft die
                  Liste gegen den tatsaechlichen Repository-Inhalt, statt
                  gegen eine zweite von Hand gepflegte Aufzaehlung.
                  ⚠ Wer von einer aelteren Version aktualisiert, bekommt mit
                  diesem Update erstmals den aktuellen Bericht-Stil.
                - CHORE: Entwicklung und Auslieferung sind getrennt. Dieses
                  Repository enthaelt nur noch, was zum Betrieb noetig ist;
                  Historie, Tests und Arbeitsdokumente liegen privat. Fuer
                  bestehende Installationen aendert sich nichts -- der
                  Updater zeigt unveraendert hierher.
                - Katalog unveraendert bei 77 Regeln. Suite 336 Tests,
                  319 gruen / 17 vorbestehend rot.

                Version 2.10.3
                - FIX: AD-03 (AD-Papierkorb) meldete eine FEHLGESCHLAGENE Abfrage
                  als echten Befund. Die Erhebung startete auf "aus" und stieg
                  nur bei Erfolg auf "an" -- fehlende Berechtigung oder ein
                  nicht erreichbarer Domaenencontroller waren damit von einem
                  echten "Papierkorb ist aus" nicht zu unterscheiden. AD-03 ist
                  Priority High, es entstand also ein Falschbefund hoher
                  Prioritaet. Der Gegenfall war ebenso falsch: ein unbekannter
                  Wert ergab PASS MIT Messwert, also ein bestandenes Verdikt
                  fuer eine nie erfolgte Messung. Beides meldet jetzt
                  NOT_CHECKED mit Grund. ⚠ Die Abgrenzung ist eng: nur wenn die
                  Abfrage das Feature gar nicht liefert, gilt der Zustand als
                  unbekannt -- leere oder unpassende EnabledScopes bleiben ein
                  echter Befund.
                - FIX: AD-FSMO-08 liest denselben Wert mit und ging bei
                  unbekanntem Papierkorb still als PASS durch. Meldet jetzt
                  ebenfalls NOT_CHECKED, aber nur dort, wo der Papierkorb den
                  Ausschlag gibt.
                - FIX: DNS-06 meldete PASS, wenn der serverweite
                  Scavenging-Schalter nicht ermittelbar war -- "in Ordnung",
                  wo niemand nachgesehen hatte. Jetzt NOT_CHECKED mit Grund.
                  (DNS-01 wurde in 2.10.2 auf NOT_APPLICABLE umgestellt; im
                  Unbekannt-Fall stehen jetzt beide auf einem Nicht-Ergebnis,
                  mit verschiedenen Status-Werten und Gruenden -- Absicht.)
                - FIX: Kennwortkomplexitaet und AD-Papierkorb werden in der
                  HTML-Uebersicht typbewusst ausgewertet, wie seit 2.10.1 im
                  Verdikt. Latent, kein aktiver Fehler: die Erhebung liefert
                  bool. Der Papierkorb bekommt zusaetzlich den dritten Zustand
                  "nicht ermittelbar".
                - ⚠ FUER KONSUMENTEN: Vier Regeln koennen jetzt einen
                  Nicht-Ergebnis-Status tragen, wo vorher PASS oder FAIL stand
                  (AD-03, AD-FSMO-08, DNS-06 und, seit 2.10.2, DNS-01).
                  scoreRelevant sinkt entsprechend -- aber NUR, wo bisher
                  geraten wurde. Wo die Erhebung gelingt, aendert sich nichts.
                  Details: docs/DASHBOARD-VERTRAG-v4.md, Nachtraege S und S2.
                - Katalog unveraendert bei 77 Regeln. Suite 330 Tests,
                  313 gruen / 17 vorbestehend rot.

                Version 2.10.2
                - FIX: DNS-01 (Zonen-Scavenging) meldete PASS, wo die Regel gar
                  nichts pruefen konnte. Scavenging hat zwei Schalter: den
                  serverweiten Vorgang, der loescht, und das Aging je Zone, das
                  Datensaetze ueberhaupt erst loeschbar macht. DNS-01 fragt
                  "Server aktiv, einzelne Zonen aus?" -- ist der serverweite
                  Schalter aus oder nicht ermittelbar, ist die Frage gar nicht
                  gestellt. Das Verdikt lautet jetzt NOT_APPLICABLE mit Grund
                  statt eines beleglosen PASS. ⚠ FUER KONSUMENTEN SICHTBAR:
                  DNS-01 wandert damit in die Ausschluesse, scoreRelevant
                  sinkt um eins. Der serverweite Zustand ist unveraendert
                  Sache von DNS-06.
                - DOCS: Nachtrag R2 im Dashboard-Vertrag -- AffectedValues ist
                  selbsttragend, AffectedItems darf daneben leer sein (gilt
                  seit jeher auch fuer BK-01/BK-02), und bei ENT-01 traegt
                  "item" bewusst einen Wert statt eines Bezeichners.
                  AdminSdHolderFormerMember in die hintKey-Tabelle nachgetragen.
                - Katalog unveraendert bei 77 Regeln. Suite 293 Tests,
                  276 gruen / 17 vorbestehend rot.

                Version 2.10.1
                - FIX: PWD-02 meldete auf jedem echten Domaenencontroller
                  "Kennwortkomplexitaet deaktiviert", OBWOHL sie aktiviert war.
                  Ursache war ein Vergleich gegen die Zeichenkette "False":
                  steht links ein echtes [bool], wandelt PowerShell den rechten
                  Operanden um, und [bool]"False" ist $true. Der Vergleich
                  wertet den Wert jetzt typbewusst aus. Sichtbar nur gegen eine
                  echte Domaene — die Mock-Daten lieferten fest $false, also
                  genau den Fall, in dem der alte Ausdruck zufaellig richtig lag.
                  BEREITS AUSGELIEFERTE BERICHTE koennen diesen Falschbefund
                  enthalten.
                - FEAT: Ein bestandenes Verdikt belegt jetzt, dass tatsaechlich
                  gemessen wurde. Bisher trugen 46 von 56 PASS-Verdikten weder
                  ActualValue noch AffectedValues und waren im Upload-JSON nicht
                  von einer Pruefung zu unterscheiden, die gar nichts gemessen
                  hatte. Betroffen waren DCDIAG, FSMO, DCSystem, Services,
                  Backup, Sites, DNS und Entra. Rein additiv, schemaVersion
                  bleibt 4 — der Konsument braucht keine Aenderung. Neue
                  Einheiten: Zones, Records, Sites, SiteLinks, Services.
                - FEAT: ENT-01 traegt Ist- und Soll-Version jetzt strukturiert
                  in AffectedValues (bestehende Form {item, hintKey}, keine
                  Vertragserweiterung; ActualValue bleibt null, weil eine
                  Versionszeichenkette dort ein Typbruch waere). DNS-07 listet
                  alle unsignierten Zonen statt nur bis zu drei. DNS-06 nennt
                  den abgefragten DNS-Server.
                - FIX: NOT_CHECKED traegt keinen Messwert mehr. Bei OS-01 und
                  den DC-*-Regeln stand bisher "0 von 1 bestaetigt" neben einem
                  Verdikt, das gerade sagt, dass nichts ermittelt werden konnte.
                  Ebenso entfaellt bei AD-FSMO-08 der ExpectedValue, der dem
                  eigenen Verdikt widersprach, sobald der AD-Papierkorb
                  aktiviert war — also bei der empfohlenen Einstellung.
                - FIX: Write-ADHCLog bricht einen Berichtslauf nicht mehr ab,
                  wenn die Protokolldatei gesperrt ist (Virenscanner, Editor,
                  zweiter Lauf). Kurze Wiederholungsversuche, danach ein
                  Hinweis statt eines Abbruchs — der Bericht ist der Zweck,
                  das Protokoll ist Beiwerk.
                - FIX: Die Testsuite ist unter gleichzeitigen Laeufen
                  reproduzierbar. Ihre Temp-Wurzeln lagen prozessuebergreifend
                  fest und wurden rekursiv geloescht; zwei Laeufe rissen
                  einander das Ausgabeverzeichnis weg, sichtbar als wechselnde,
                  voellig unbeteiligte Fehlschlaege.
                - Katalog unveraendert bei 77 Regeln. Suite 284 Tests,
                  267 gruen / 17 vorbestehend rot. Details:
                  docs/OFFENE-PUNKTE-PASS-BELEGE.md,
                  docs/DASHBOARD-UEBERGABE.md (Konsumentenvertrag).

                Version 2.10.0
                - FEAT: Der HTML-Bericht kennt jetzt einen dritten Anzeigezustand
                  fuer nicht ermittelbare Pruefungen (bisher zeigte er bei PWD-02
                  faelschlich "Deaktiviert", wo die Regel NOT_CHECKED meldet).
                  Ein eigener Abschnitt "Nicht ermittelbare Pruefungen" fasst
                  diese Faelle jetzt im Bericht zusammen — bisher stand
                  NOT_CHECKED nur im Upload-JSON.
                - FIX: SEC-09 (privilegierte Konten) schneidet die als
                  AdminSDHolder-geschuetzt markierte Teilmenge jetzt gegen die
                  AKTUELLE Gruppenmitgliedschaft. AD setzt adminCount beim
                  Gruppenaustritt nicht zurueck; ehemals privilegierte Konten
                  erschienen bisher weiterhin als geschuetzt, obwohl gerade sie
                  die interessanten Faelle sind. Neuer Hinweistext bei
                  betroffenen Konten.
                - FEAT: AD-FSMO-07 prueft jetzt zusaetzlich die Domaenenzahl im
                  Forest (Get-ADForest), nicht mehr nur die Rollenverteilung.
                - FEAT: Die FSMO-Erreichbarkeitspruefung versucht Port 636
                  (LDAPS) als Rueckfall, wenn Port 389 nicht erreichbar ist.
                - FIX: DCDIAG und OS-01 behandeln einen gemischten Zustand
                  (ein Server ermittelbar, der andere nicht) jetzt gleich.
                - FEAT: Das Upload-JSON traegt einen neuen, rein additiven
                  Block scoreSummary mit den bereits bereinigten Kennzahlen
                  (Gesamtzahl, Aufschluesselung nach Status und Prioritaet, die
                  Ausschlussgruende getrennt, die score-relevante Zahl) — das
                  auswertende System muss dafuer nicht mehr selbst filtern.
                  Fuer Dashboards die sichtbarste Neuerung dieser Version.
                - FIX: Die Undetermined-Listen bei OS-01 und DCDIAG sind wie
                  die bestehende PII-Kappung jetzt auf 50 Eintraege gekappt;
                  die Gesamtzahl bleibt im SkipReason erhalten.
                - Bewusst NICHT umgestellt: DNS-03 bleibt bei ICMP als
                  Zusatzsignal (die Dienstabfrage ist die massgebliche
                  Komponente) — fachlich geprueft, siehe docs/OFFENE-PUNKTE.md.
                - Katalog unveraendert bei 77 Regeln. Suite unveraendert bei
                  165 gruen / 17 vorbestehend rot (182). Details:
                  docs/OFFENE-PUNKTE.md, docs/DASHBOARD-UEBERGABE.md
                  (Konsumentenvertrag).

                Version 2.9.0
                - FEAT: SITE-01 wertet ein Site-Link-Replikationsintervall ohne
                  ausdruecklich gesetzten Wert ("Default") jetzt als 180 Minuten
                  statt es zu uebergehen — der AD-Standard, bisher stillschweigend
                  ignoriert. Neue Befunde moeglich, wo bisher keiner erschien.
                - FEAT: SITE-04 (Standort ohne Global Catalog) in zwei Regeln
                  getrennt. SITE-04 behaelt den echten Konfigurationsfehler
                  (Medium); der bisher mitgezaehlte harmlose Fall — Standort ganz
                  ohne Domaenencontroller — heisst jetzt SITE-06 und laeuft auf
                  Prioritaet Info. Bestehende SITE-04-Befunde koennen dadurch als
                  SITE-06 erscheinen.
                - FEAT: "Nicht ermittelbar" zaehlt nicht mehr als bestanden.
                  Betroffen: KRBTGT-Passwortalter (AD-04) ohne Leserecht,
                  Kennwortkomplexitaet (PWD-02) mit $null-Wert, unbekanntes
                  Betriebssystem (OS-01), nicht erkanntes DCDIAG-Testergebnis (alle
                  18 bisherigen DC-Regeln). Alle vier melden jetzt NOT_CHECKED mit
                  Grund statt eines stillen PASS. Kein Schema-Bump — bewusste
                  Entscheidung, da schemaVersion 4 gerade erst eingefuehrt wurde.
                - FEAT: Drei ausgefuehrte, bisher nie ausgewertete dcdiag-Tests
                  bekamen Regeln: DC-CONN, DC-ADVT, DC-FRS (alle High).
                - FEAT: AD-FSMO-01 bis 05 pruefen die Erreichbarkeit jetzt per
                  TCP-Verbindung auf Port 389 (LDAP) statt per ICMP-Ping — robust
                  gegen Umgebungen, die ICMP blockieren, aber LDAP zulassen.
                - FIX: Toter Condition-Ausdruck bei PWD-04 an den tatsaechlich
                  verwendeten Threshold angeglichen (Code unveraendert, SEC-05
                  war bereits in der Vorcharge dran).
                - DOCS: dcdiag-Parser bekam einen sprachunabhaengigen Rueckfall
                  fuer alles, was nach dem englischen Muster noch "Unknown" ist.
                  ACHTUNG: das deutsche Muster ist unbelegt (aus Forenzitaten,
                  kein echter deutschsprachiger Server) — im Code wie im Test so
                  gekennzeichnet.
                - Ausserdem enthalten (schon vorher fertig, aber noch nie
                  veroeffentlicht — ein Anwender bekommt sie beim Sprung von
                  2.8.0 auf 2.9.0 zum ersten Mal):
                - FEAT: DNS-06/DNS-07 zaehlen nicht mehr als Fehlschlag in der
                  Punktzahl. Beide sind laut eigenem Text nur ein Hinweis, liefen
                  bisher aber wie ein echter Mangel ein — bei fast jeder Umgebung.
                  Wer die Punktzahl ueber die Zeit vergleicht, sieht dadurch einen
                  Sprung nach oben ohne Aenderung an der Umgebung.
                - FEAT: SEC-09 weist zusaetzlich aus, wie viele der gemeldeten
                  Konten AdminSDHolder-geschuetzt sind ("davon ...: n"). Gesamtzahl
                  im Befund unveraendert.
                - FIX: Die Ampel-Pillen fuer Organisations- und Schema-Admins
                  faerbten bisher erst ab 6 Mitgliedern rot, obwohl die Regeln
                  schon ab 3 bzw. 2 Mitgliedern feuern. Pille und Befund zeigen
                  jetzt dasselbe.
                - FIX: Lange Kontonamen (DN) liefen in den Konto-Tabellen ueber
                  und ueberschrieben die Status-Spalte; sie brechen jetzt in der
                  eigenen Zelle um.
                - Katalog waechst von 73 auf 77 Regeln. Suite von 94 auf 136
                  gruen (17 vorbestehend rot, unveraendert). Details:
                  docs/OFFENE-PUNKTE.md (Teil 3, 5, 6),
                  docs/DASHBOARD-VERTRAG-v4.md (Konsumentenvertrag).

                Version 2.8.0
                - FEAT: Der HTML-Bericht zeigt je Befund die neuen Katalogfelder
                  Requirement ("Anforderung"), Description ("Hintergrund") und
                  Impact ("Auswirkung") aus recommendations.json — als
                  aufklappbarer Block (details open, kein JavaScript), in beiden
                  Sprachen. Fehlt ein Feld, entfaellt nur dessen Zeile; eine
                  aeltere recommendations.json laeuft unveraendert weiter.
                - FEAT: Upload-JSON schemaVersion 4 — Verdikte sind zweisprachig.
                  ACHTUNG, NICHT additiv: Detail, SkipReason, Category und
                  SubCategory sind jetzt {de,en}-Objekte statt Strings. Das Feld
                  language sagt nur noch, in welcher Sprache der HTML-Bericht
                  erzeugt wurde — die Verdikte liegen immer in beiden vor.
                  Handlungsliste fuer das auswertende System (C1-C4 VOR dem
                  Rollout): docs/DASHBOARD-VERTRAG-v4.md.
                - FEAT: AffectedItems enthaelt bei SRV-01, BK-01/02, REP-01,
                  EVT-01/02 und SITE-01/04/05 nur noch sprachneutrale
                  Bezeichner; Messwert und Grund stehen daneben im neuen Feld
                  AffectedValues ({item,value,unit} bzw. {item,hintKey}).
                  value=null heisst "nicht ermittelbar", nicht 0. Die
                  Befundsaetze im HTML-Bericht sind unveraendert.
                - FIX: Der Verdikt-Aufruf bekam die Thresholds aus settings.json
                  nicht durchgereicht — bei angepasstem KrbtgtPasswordAgeDays
                  meldete der HTML-Bericht FAIL und das Upload-JSON PASS fuer
                  dieselbe Regel im selben Lauf.
                - FIX: REP-01/EVT-01 bezogen Status (settings.json) und
                  ExpectedValue (Katalog) aus verschiedenen Quellen; bei
                  uebersteuerten Schwellen nannte das Verdikt eine Grenze,
                  gegen die nicht gemessen wurde. Katalogwert bleibt Rueckfall.
                - FIX: SITE-05 haette beim Entfernen des uebersetzten
                  Platzhalters lautlos nie mehr gefeuert (Ausloesung hing an der
                  Listenlaenge); jetzt entkoppelt und in beide Richtungen
                  getestet. Category ist im Katalog jetzt {de,en} (73 Regeln).
                - TEST: Pester-Suite lief nicht (0 von 72, Discovery-Scoping und
                  fehlender Utils-Import); jetzt 87 Tests gruen. Neu:
                  tests/Test-ADHCCatalog.ps1 (Katalogproben ohne Domaene,
                  cp1252/ss/Sternchen/leer/vergessene Uebersetzung) und
                  tests/New-ADHCPreviewReports.ps1 (Berichte aus Mock-Daten,
                  je Sprache ein Unterverzeichnis).

                Version 2.7.8
                - FIX: REP-01 und REP-02 konnten in einer Domaene mit nur EINEM
                  Domaenencontroller nie bestehen. Ohne zweiten DC gibt es keinen
                  Replikationspartner; Get-ADReplicationPartnerMetadata scheitert
                  bzw. liefert nichts, und der Eintrag landete als "Unreachable"
                  in REP-02 — ein Befund, den niemand beheben kann. Solche Faelle
                  tragen jetzt Status "NotApplicable" (neu im Collector) und die
                  beiden Regeln werden uebersprungen, statt zu feuern.
                - FEAT: Upload-JSON schemaVersion 3 — neuer Verdikt-Status
                  NOT_APPLICABLE plus Feld SkipReason. ACHTUNG: der Status-Wert ist
                  nicht rein additiv. Konsumenten, die nur PASS/FAIL/NOT_CHECKED
                  kennen, muessen NOT_APPLICABLE wie NOT_CHECKED behandeln — nicht
                  bestanden, aber ausdruecklich KEIN Fehler.
                - Unterscheidung bewusst nicht allein an der DC-Anzahl: in einem
                  Forest mit mehreren Domaenen repliziert auch ein einzelner
                  Domaenen-DC die Configuration-/Schema-Partition. Massgeblich ist,
                  ob die Abfrage tatsaechlich Partner geliefert hat.

                Version 2.7.7
                - FIX: DNS-01 meldete dauerhaft ein nicht behebbares Finding. Die
                  automatisch angelegten Reverse-Systemzonen 0.in-addr.arpa,
                  127.in-addr.arpa und 255.in-addr.arpa fuehren kein Aging und
                  koennen es auch nicht — sie landeten aber in MissingScavenging
                  und damit als "Scavenging zone mismatch" im Report und im
                  JSON-Export. Diese drei Zonen sind jetzt aus der
                  Scavenging-Bewertung (MissingScavenging, ScavengingUnknown,
                  ScavengingMeasured) ausgenommen; in der Zonentabelle tragen sie
                  die neutrale Pille "Nicht anwendbar (Systemzone)".

                Version 2.7.6
                - FIX: Die Kachel "Scavenging (Aging)" hat nie etwas gemessen. Der Check
...
function Invoke-ADHCUpdateCheck {
    ...
    try {
        ...
        $updateFiles = @{
            ...
            # report.style.css wird zur LAUFZEIT gebraucht (Reporting.psm1:38 liest
            # sie und inliniert sie in ein <style>-Element), fehlte hier aber bis
            # 2.10.4 -- jede Stilaenderung blieb bei ausgelieferten Installationen
            # unsichtbar. Die Logos ebenso: config/settings.json verweist mit
            # LogoUrl standardmaessig auf logo-lake-transparent.png.
            # Binaerdateien sind unbedenklich: Invoke-WebRequest -OutFile schreibt
            # roh, und die Pruefungen unten greifen nur bei .ps1/.psm1/.json.
            "templates/report.style.css"           = Join-Path $ScriptRoot "templates\report.style.css"
            "templates/logo-lake-transparent.png"  = Join-Path $ScriptRoot "templates\logo-lake-transparent.png"
            "templates/logo-lake.png"              = Join-Path $ScriptRoot "templates\logo-lake.png"
        }
...
function Start-Analysis {
    ...
    if ($script:UseMockData) {
        ...
        $ouSecData = if ($Selection.OUAccountSecurity) {
            ...
            # U3/SEC-09: $secData.ProtectedGroupMemberDNs uebergeben, damit die
            # ACL-Analyse adminCount=1-Konten gegen die AKTUELLE Mitgliederliste
            # der geschuetzten Gruppen abgleichen kann. $secData wird weiter oben
            # (vor $ouSecData) berechnet -- die Reihenfolge stimmt bereits. Ist
            # $secData $null (Security-Sektion nicht ausgewaehlt), bleibt die
            # Unterscheidung "aktuell/ehemals" in Get-ADOUAndAccountSecurity
            # konservativ nicht ermittelbar (dort dokumentiert).
            Get-ADOUAndAccountSecurity -Settings $Settings -ProgressCallback $progressCallback `
                -ProtectedGroupMemberDNs $secData.ProtectedGroupMemberDNs
        } else { $null }
