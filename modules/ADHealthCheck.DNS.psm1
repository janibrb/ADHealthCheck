...
# Automatisch angelegte Reverse-Systemzonen. Windows erzeugt sie beim Einrichten
# des DNS-Dienstes; Aging/Scavenging laesst sich dort weder aktivieren noch
# sinnvoll anwenden (sie enthalten nur statische Systemeintraege). Sie sind
# deshalb aus jeder Scavenging-Bewertung ausgenommen — sonst meldet der Report
# dauerhaft drei "Fehler", die niemand beheben kann.
$script:ADHCSystemReverseZones = @('0.in-addr.arpa', '127.in-addr.arpa', '255.in-addr.arpa')
...
function Select-ADHCNameserverZone {
    ...
    $systemZones     = @($script:ADHCTrustAnchorZone, '.') + $script:ADHCSystemReverseZones
...
function Get-ADHCServerScavenging {
    ...
    .PARAMETER Server
        Name des Servers, von dem der Schalter gelesen wird -- das Abfrageziel
        des DNS-Laufs. Er wird als Feld Server im Rueckgabeobjekt abgelegt,
        damit DNS-06 den betroffenen Server benennen kann. Bis P34 war der
        Wert NUR waehrend des Aufrufs bekannt und ging danach verloren: die
        Regel konnte keinen Server ausweisen, und die Nameserver der Zone
        ($dns.NSStatus) sind nicht dasselbe -- sie stattdessen zu nennen hiesse,
        einen Bezeichner zu erfinden. Bleibt der Parameter leer, bleibt auch
        das Feld leer; ein erfundener oder leerer Bezeichner waere schlechter
        als keiner.
    #>
    [CmdletBinding()]
    param(
    ...
    # Leerzeichenkette und $null sind beide "nicht bekannt" -- vereinheitlichen,
    # damit die auswertende Seite nur EINEN Fall pruefen muss.
    $serverName = if ([string]::IsNullOrWhiteSpace($Server)) { $null } else { $Server }
    ...
    try {
        ...
        # Der Name kommt auch hier mit: das Abfrageziel ist bekannt, auch wenn
        # die Abfrage scheitert. Nicht ermittelbar ist der SCHALTER, nicht der
        # Server.
        return [PSCustomObject]@{ Enabled = $null; IntervalHours = $null; Server = $serverName }
    }
}
...
function Get-ADDNSHealthStatus {
    ...
    try {
		...
		# 0/127/255.in-addr.arpa bleiben komplett aussen vor: dort ist Scavenging
		# technisch nicht aktivierbar, ein Finding waere nicht behebbar.
		$allTestedZones         = @($forwardZones) + @($reverseZones)
		$scavengingScope        = @($allTestedZones | Where-Object {
			$script:ADHCSystemReverseZones -notcontains ("$($_.ZoneName)").Trim().ToLower()
		})
		$zonesWithoutScavenging = @($scavengingScope | Where-Object { $_.AgingEnabled -eq $false } | ForEach-Object { $_.ZoneName })
		$scavengingUnknown      = @($scavengingScope | Where-Object { $null -eq $_.AgingEnabled } | ForEach-Object { $_.ZoneName })
		$scavengingMeasured     = @($scavengingScope | Where-Object { $null -ne $_.AgingEnabled }).Count

		# Serverweiter Schalter: ohne ihn ist jede Zonenkonfiguration wirkungslos.
		# -Server: der Schalter wird von GENAU EINEM Server gelesen, dem
		# Abfrageziel dieses Laufs. Sein Name gehoert mit ins Rueckgabeobjekt,
		# sonst kann DNS-06 den betroffenen Server nicht benennen (P34).
		$serverScavenging = Get-ADHCServerScavenging -CimArgs $dnsCn -Server $TargetServer
